...
func (api *Client) UpdateMessageWithAttachments(ctx context.Context, channel string, params UpdateMessageParameters) (string, string, string, error) {
	values := url.Values{
	...
	response, err := chatRequest(ctx, "chat.update", values, api.debug)
	if err != nil {
		return "", "", "", err
	}
	return response.Channel, response.Timestamp, response.Text, nil
...
func (api *Client) Unfurl(ctx context.Context, channel string, params UnfurlParameters) error {
	values := url.Values{
		"token":   {api.config.token},
		"channel": {channel},
		"ts":      {params.Timestamp},
	...
	_, err := chatRequest(ctx, "chat.unfurl", values, api.debug)
	if err != nil {
		return err
	}
	return nil
